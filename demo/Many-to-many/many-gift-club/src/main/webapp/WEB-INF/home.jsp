...
<body>
	<div class="container mt-5">
		...
		<a href="/gifts/new"> Send a new gift</a> <a href="/logout">
			Logout</a>
		<h1>All gifts</h1>
		<table class="table">
			<thead>
			...
			<tbody>
				<c:forEach var="gift" items="${gifts }">
					<tr>
						<td>
							${gift.likedUsers.size()}
						<c:choose>
								<c:when test="${ gift.likedUsers.contains(user)}">
									<form action="/gifts/${gift.id }/unlike" method="post">
										<input type="hidden" name="_method" value="put" />
										<button class="btn btn-danger">unLike!</button>
									</form>
								</c:when>
								<c:otherwise>
									<form action="/gifts/${gift.id }/like" method="post">
										<input type="hidden" name="_method" value="put" />
